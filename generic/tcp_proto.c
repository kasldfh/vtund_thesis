...
{
     vtun_syslog(LOG_ERR, "Queue size in tcp_write is: %d\n", queue.size);
     printf("Queue size in tcp_write is: %d\n", queue.size);
     register char *ptr;
    ...
    //TODO: make it thread safe
     ...
     queue.arr[queue.size+1] = queue.size+1;
     queue.size += 1;

     //for now we will pretend that write_n never has any errors....
