...
void * traffic_handler (void * queue_ptr) {
    struct Queue *queue = (struct Queue *) queue_ptr;
...
int main(int argc, char *argv[], char *env[])
{
    ...
    printf("Logging now!\n");
    vtun_syslog(LOG_ERR, "HELLO WORLD!\n");
    ...
    if( daemon ){
        if( dofork && fork() ) {
            printf("About to exit after forking\n");
            exit(0);
        }
    ...
    pthread_t handler;
    if(pthread_create(&handler, NULL, traffic_handler, &queue)) {
